        ...
        with lock:
            ...
            if definitions is not None:
                if definitions:
                    ...
                    provision.setdefault("_empty_def_paragraph_indices", []).append(unit.paragraph_index)
                n_resolved += 1
    ...
    for item in queue:
        ...
        if item["reason"] == "validation_failed":
            ...
            if not grounded and item.get("paragraph_index") is not None:
                provision.setdefault("_empty_def_paragraph_indices", []).append(item["paragraph_index"])
